...
import com.nirmal.medapp.modal.AppointmentItemType;
import com.nirmal.medapp.modal.AppointmentItems;
import com.nirmal.medapp.modal.Doctor;
import com.nirmal.medapp.modal.Lab;
import com.nirmal.medapp.modal.Patient;
import com.nirmal.medapp.modal.Person;
import com.nirmal.medapp.modal.PersonType;
import com.nirmal.medapp.modal.Prescription;
...
public class DatabaseHelper {
	...
	public int createAppointment(Appointment appointment) {
		try {
			...
			ps.executeUpdate();
			ps = connection
					.prepareStatement("Select appointmentId from appointment order by appointmentId desc LIMIT 1");
			ResultSet rs = ps.executeQuery();
			// create an item
			if (rs.next()) {
	...
	public int addItemInAppointment(int id, int type, String description) {
		try {
			...
			ps = connection.prepareStatement("Select itemId from appointmentItems order by itemId desc LIMIT 1");
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
	...
	public ArrayList<ArrayList<Appointment>> getAppointments(int personId, String mactingColumn) {
		...
		try {
			...
			if (mactingColumn != null) {
				...
				// from receptionist
				ps = connection.prepareStatement("select * from appointment  order by dateCreated desc");
			}
	...
	public ArrayList<AppointmentItems> getAppointmentsItems(int id) {
		...
		try {
			...
			while (itemSet.next()) {
				AppointmentItems item = new AppointmentItems(itemSet.getInt("itemId"), itemSet.getLong("date"),
						itemSet.getInt("typeId"), itemSet.getString("description"), itemSet.getInt("appointmentId"));
				items.add(item);
			}
			return items;
		} catch (SQLException e) {
	...
	public Appointment getDetailedAppointment(int appointmentId) {
		try {
			...
			if (rs.next()) {
				...
				Patient patient = getPatient(patientId);
				Appointment appointment = new Appointment(id, doctor, rs.getString("title"), rs.getLong("dateCreated"),
						rs.getString("symptons"), rs.getString("disease"), patient, rs.getLong("allocatedDate"),
						rs.getLong("preferredDate"), rs.getInt("isClosed"));
	...
	public ArrayList<PersonType> getPersonType() {
		try {
			...
			while (rs.next()) {
				PersonType personType = new PersonType(rs.getInt("typeId"), rs.getString("description"),
						rs.getString("type"));
	...
	public ArrayList<Appointment> getAppointment() {
		try {
			...
			while (rs.next()) {
				...
				Appointment appointment = new Appointment(rs.getInt("appointmentId"), doctor, patient,
						rs.getInt("isClosed"), rs.getLong("preferredDate"), rs.getLong("allocatedDate"),
						rs.getLong("dateCreated"), items, rs.getString("symptons"), rs.getString("disease"),
						rs.getString("title"));
	...
	public int getAppointmentCount(int personId, String matchingColumn) {
		...
		try {
			ps = connection
					.prepareStatement("select COUNT(*) AS rowcount from appointment where " + matchingColumn + " =?");
			ps.setInt(1, personId);
	...
	public int getPatientCount(int personId) {
		...
		try {
			ps = connection.prepareStatement(
					"select COUNT(DISTINCT patientId) AS rowcount from appointment where doctorId =?");
			ps.setInt(1, personId);
	...
	public ArrayList<AppointmentItemType> getAppointmentItemType() {
		try {
			...
			while (rs.next()) {
				AppointmentItemType appointmentItemType = new AppointmentItemType(rs.getInt("typeId"),
	...
	public ArrayList<Prescription> getPrescription() {
		try {
			...
			while (rs.next()) {
				Prescription prescription = new Prescription(rs.getInt("medicineId"), rs.getString("medicineName"),
						rs.getDouble("quantity"), rs.getInt("times"), rs.getInt("itemId"));
		...
		try {
			...
			while (rs.next()) {
				Lab lab = new Lab(rs.getInt("labId"), rs.getString("labName"), rs.getString("testFor"),
						rs.getString("labResult"), rs.getString("reportFile"), rs.getInt("itemId"),
	...
	public int addLab(String labName, String testFor, int itemId, int doctorId) {
		try {
			PreparedStatement ps = connection
					.prepareStatement("insert into lab (labName, testFor, itemId, doctorId) values(?,?,?,?)");
	...
	public int addPrescription(String medicineName, Double quantity, int times, int itemId) {
		try {
			PreparedStatement ps = connection
					.prepareStatement("insert into prescription (medicineName,quantity,times,itemId) values(?,?,?,?)");
	...
	public ArrayList<Staff> getStaff() {
		try {
			// get person from database
			PreparedStatement ps = connection
					.prepareStatement("select personId from person where personType<>2 AND personType<>1");
			ResultSet rs = ps.executeQuery();
	...
	public int allocateDoctor(int appointmentId, int doctorId, long allocatedDate) {
		try {
			// update appointment
			PreparedStatement ps = connection
					.prepareStatement("update appointment set doctorId=?, allocatedDate=? where appointmentId=?");
			ps.setInt(1, doctorId);
			ps.setLong(2, allocatedDate);
			ps.setInt(3, appointmentId);
			int status = ps.executeUpdate();
			if (status > 0) {
				return addItemInAppointment(appointmentId, 6,
						"Date Allocated :- " + DateUtils.getStringFromDate(allocatedDate));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public int updateLabReport(int labReportId, String result, int itemId) {
		try {
			...
			PreparedStatement ps = connection
					.prepareStatement("SELECT appointmentId from appointmentItems where itemId=?");
			ps.setInt(1, itemId);
			ResultSet rs = ps.executeQuery();
	...
	public int closeAppointment(int appointmentId, String by) {
		try {
			// update appointment
			PreparedStatement ps = connection
					.prepareStatement("update appointment set isClosed=1 where appointmentId=?");
			ps.setInt(1, appointmentId);
